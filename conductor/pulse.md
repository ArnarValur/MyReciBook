...
- Storage placeholder-armed — activation NEXT SESSION (Arnar, 2026-08-06
  dawn): full walkthrough docs/storage-creds-runbook.md (supersedes chat
  spec); two dev.env lines flip it live (rule 6).
...
- Storage activation ← Arnar's console pass NEXT SESSION, ~35 min —
  walkthrough: docs/storage-creds-runbook.md.
- Billing 3g ← Play fee ← Arnar's hands-on verdict on the S21 build.

## 📋 Next queue (sequence — no schedule)
1. Arnar: hands-on pass on the S21 (everything live incl. turn 6) →
   strong-build verdict → fee $25 if met → Code wires billing 3g (D10,
   license testers).
2. Next session, Arnar: creds per runbook → dev.env → storage live →
   first true Drive/Dropbox sync smoke on device.
