# Tracks — MyReciBook
*One line per track. Detail lives in conductor/tracks/<id>/plan.md, created by
/new-track when the track opens. Build order is fixed: extract → save → list → open.*

| ID | Track | Status | One-liner |
|---|---|---|---|
...
| T3 | mvp-build | 2026-08-15 — Unlock tab (queue tab retired, Arnar's call) + cookbook grid ⇄ list toggle, both FOLDED TO MAIN; Drive retry still pending | Next: Drive retry → smoke PASSED → dev.env build to S21 → cover-flow tests → fee → billing 3g; alpha 19 Oct |
